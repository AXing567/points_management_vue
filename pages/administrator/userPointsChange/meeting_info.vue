<template>
	<view>
		...
		<!-- 
		<uni-section title="上传图片(请确保无重复图片,否则请删除重添加)" type="line" v-show="check!=1">
			<view class="example-body">
				<uni-file-picker limit="9" title="最多选择9张图片" @select="select" :autoUpload="false"
					@delete="del_img"></uni-file-picker>
			</view>
		</uni-section> -->
		<button @click="upload_meeting"
			:disabled="check!=2">{{check == 0? '未审核': check == 1 ? '已通过' : '未通过(提交修改)'}}</button>
	...
	export default {
		data() {
			return {
				...
				var id = getApp().globalData.index;
				var meeting = getApp().globalData.meetings[id];
